...
class PlanExecutor:
    ...
    async def async_stop(self, restore_auto: bool = True) -> None:
        ...
        if restore_auto and self._last_applied not in (None, ACTION_AUTO):
            await self._call_script(ACTION_AUTO, 0.0)
    ...
    async def async_apply_current(self) -> None:
        ...
        if slot is None or not coordinator.data or not coordinator.data.valid:
        ...
        await self._call_script(action, slot.charge_power_w)
        self._last_applied = action
    ...
    async def _call_script(self, action: str, power_w: float) -> None:
        key = {
